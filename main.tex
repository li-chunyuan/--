...
\def\allFiles{1}% 定义一个标志变量容器，标志现在时主文件编译还是子文件独立编译


include{latex/config}		% 导入所需宏包	


%==========================部分调试编译区=============================

\includeonly{latex/cover}	% 只编译filelist.tex文件，调试时使用

%其中“latex/”的作用是告诉LaTeX去子目录 latex/ 里找 cover.tex 文件
...
\include{latex/cn_abstract}				% 导入中文摘要  
\include{latex/en_abstract}				% 导入英文摘要
...
%% -------------------- 正文 -------------------- 				
\include{latex/Chapter}                 % 载入章节内容
\backmatter                             	% 关闭章节序号，对页码没有影响

%% -------------------- 谢辞 -------------------- 	
\include{latex/Thanks}                  % 导入谢辞
